<script lang="ts">
  ...
  const flash = async () => {
    ...
    for (const [frameIdx, frameEntry] of frameEntries.entries()) {
      ...
      const actualSlotsLen = getFrameWidgetCount(
        frameEntry.frameLayouts,
        layout
      )
    ...
    await tryCmd(commands.flashMemoriState(draft)).match(
      () => {
        errMsg = ''
      },
      (error) => {
  ...
  const handleDragOver: DragDropEvents['dragover'] = (event) => {
    ...
    const baseFrame = useCommittedFrame
      ? committedFrame
      : (dragFrame ?? committedFrame)
