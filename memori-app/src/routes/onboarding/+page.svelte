<script lang="ts">
  ...
  let isLoggedInGithub = $derived(
    authState.usersByProvider['github'] !== undefined
  )
  let isLoggedInTwitch = $derived(
    authState.usersByProvider['twitch'] !== undefined
  )
  ...
  async function connect() {
    errMsg = ''
    isLoading = true
    await connectDevice('RealDevice').match(
      () => {
        // playSuccessSound()
...
<div class="page">
  <div class="content text-foreground/90">
    <div
      class="layout-grid grid h-full grid-rows-[minmax(0,1fr)_auto] px-4 pt-4"
    >
      <Carousel.Root
...
{#snippet slideFrame(content: any, actions: any)}
  <section class="relative h-full min-h-0">
    <div class={stepFrameCls.topRegion}>
      {@render content()}
    </div>

    <div class={stepFrameCls.bottomRegion}>
      <div class={stepFrameCls.actionContainer}>
        {@render actions()}
      </div>
    </div>
  </section>
...
{#snippet step2Content()}
  <div class="w-full space-y-4">
    <p
      class="text-xs font-medium uppercase tracking-[0.24em] text-foreground/50"
    >
      Step 2
    </p>
    <h1
